...
class NERModel(LanguageModel):
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope("embed_layer"):
                embedding = xavier_initializer([len(self.wv), self.config.embed_size])
                embeddings = tf.nn.embedding_lookup(embedding, self.input_placeholder)
                window = tf.reshape(embeddings, shape=[-1, self.config.window_size * self.config.embed_size],
                                    name="window")
            ### END YOUR CODE
            return window
    ...
    def add_model(self, window):
        ...
        with tf.variable_scope("layer"):
            W = tf.get_variable("w", shape=[self.config.window_size * self.config.embed_size, self.config.hidden_size],
                                initializer=xavier_initializer)
            U = tf.get_variable("u", shape=[self.config.hidden_size, self.config.label_size],
                                initializer=xavier_initializer)
            b1 = tf.constant(0.0, shape=[1, self.config.hidden_size], dtype=tf.float32)
            b2 = tf.constant(0.0, shape=[1, self.config.label_size], dtype=tf.float32)

        h = tf.matmul(window, W) + b1
        tanh_activation = tf.tanh(h)
        droput_tanh = tf.nn.dropout(tanh_activation, self.dropout_placeholder)
        output = tf.matmul(droput_tanh, U) + b2

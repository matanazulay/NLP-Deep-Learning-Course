...
class RNNLM_Model(LanguageModel):
    ...
    def add_projection(self, rnn_outputs):
        ...
        with tf.variable_scope("projection"):
            self.U = tf.Variable(tf.random_uniform([self.config.hidden_size, len(self.vocab)], 0.0, 1.0))
            self.b_2 = tf.constant(0.0, shape=[1, len(self.vocab)])
        outputs = []
        for t in range(self.config.num_steps):
            outputs.append(tf.matmul(rnn_outputs[t], self.U) + self.b_2)
        ### END YOUR CODE
        return outputs
    ...
    def add_loss_op(self, output):
        ...
        loss = sequence_loss([output], [
            tf.reshape(self.labels_placeholder, [-1])],
                             [tf.ones([self.config.batch_size * self.config.num_steps])], len(self.vocab))
        ### END YOUR CODE
        return loss
    ...
    def add_model(self, inputs):
        ...
        for step in range(self.config.num_steps):
            state = tf.nn.sigmoid(tf.matmul(state, self.H) + tf.matmul(inputs[step], self.I) + self.b_1)

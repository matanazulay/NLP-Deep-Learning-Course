...
class GRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope, dtype=tf.float32):
            ...
            W_r = tf.get_variable(name = "W_r", shape = W_shape, initializer=xavier)
            U_r = tf.get_variable(name = "U_r", shape = U_shape, initializer=xavier)
            b_r = tf.get_variable(name="b_r", shape = b_shape, initializer=tf.zeros_initializer())
            W_z = tf.get_variable(name="W_z", shape=W_shape, initializer=xavier)
            U_z = tf.get_variable(name = "U_z", shape = U_shape, initializer=xavier)
            b_z = tf.get_variable(name="b_z", shape = b_shape, initializer=tf.zeros_initializer())
            W_o = tf.get_variable(name="W_o", shape=W_shape, initializer=xavier)
            U_o = tf.get_variable(name="U_o", shape=U_shape, initializer=xavier)
            b_o = tf.get_variable(name="b_o", shape=b_shape, initializer=tf.zeros_initializer())

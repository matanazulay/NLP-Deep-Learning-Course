    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(sentence_size):
            ...
            for t in range(i - window_size, i + window_size + 1):
                if t < 0:
                    cur_window += start
                elif t > sentence_size - 1:
                    cur_window += end
                else:
                    cur_window += sentence[t]
            windowed_data.append((cur_window, label))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("ner", initializer=tf.contrib.layers.xavier_initializer()):
            W = tf.get_variable("W", [self.config.n_window_features*self.config.embed_size, self.config.hidden_size])

...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        mask = [True] * sentence_size
        if sentence_size < max_length:
            sentence_length = sentence + [zero_vector] * (max_length - sentence_size)
            labels_length = labels + [zero_label] * (max_length - sentence_size)
            mask += [False] * (max_length - sentence_size)
        elif sentence_size >= max_length:
            sentence_length = sentence[:max_length]
            labels_length = labels[:max_length]
            mask = mask[:max_length]
        ret.append((sentence_length, labels_length, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable(name="b_2", shape=(self.config.n_classes),
                                  initializer=tf.contrib.layers.xavier_initializer())
        h = tf.zeros(name="h", shape=(1, self.config.hidden_size))
        ### END YOUR CODE

        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ### YOUR CODE HERE (~6-10 lines)
                output, new_state = cell(x[:, time_step, :], h)
                tf.get_variable_scope().reuse_variables()
                output_drop = tf.nn.dropout(output, dropout_rate)
                preds.append(tf.matmul(output_drop, U) + b_2)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits = masked_preds, labels= masked_labels))
        ### END YOUR CODE
        return loss

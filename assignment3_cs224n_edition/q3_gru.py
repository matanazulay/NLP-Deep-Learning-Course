...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self):
        ...
        output, _ = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32, scope=tf.get_variable_scope())
        final_state = tf.unstack(output, axis=1)[-1]
        preds = tf.nn.sigmoid(final_state)
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_sum(tf.nn.l2_loss(y - preds))
        ### END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients:
            ...
            grads, self.grad_norm = tf.clip_by_global_norm(tf.gradients(loss, vars), self.config.max_grad_norm)

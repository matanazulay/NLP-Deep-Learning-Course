...
class RNN_Model():
    ...
    def add_model(self, node):
        ...
        with tf.variable_scope('Composition', reuse=True):
            ...
            embed = tf.get_variable('embedding')
            ### END YOUR CODE
        ...
        if node.isLeaf:
            ### YOUR CODE HERE
            curr_node_tensor = tf.expand_dims(tf.gather(embed,self.vocab.encode(node.word)), 0)
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            t1 = node_tensors[node.left]
            t2 = node_tensors[node.right]
            node_concat = tf.concat([t1, t2], 1)
            #node_concat = tf.stack([node_tensors[node.left], node_tensors[node.right]], axis=1)
            curr_node_tensor = tf.nn.relu(tf.matmul(node_concat, W1) + b1)
            ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        with tf.variable_scope('Projection', reuse=True):
            U = tf.get_variable('U')
        loss += self.config.l2 * (tf.nn.l2_loss(W1) + tf.nn.l2_loss(U))
        # END YOUR CODE
        return loss

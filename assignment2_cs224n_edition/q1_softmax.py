...
def softmax(x):
    ...
    ### YOUR CODE HERE
    x_normalized = x - tf.reduce_max(x, reduction_indices=[1], keep_dims = True)
    expos = tf.exp(x_normalized)
    out = expos / tf.reduce_sum(expos, reduction_indices = [1], keep_dims = True)
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    predicitons = tf.reduce_sum(yhat * tf.to_float(y), reduction_indices = [1])
    log_pred = -tf.log(predicitons)
    out = tf.reduce_sum(log_pred)
    ### END YOUR CODE

...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings_temp = tf.nn.embedding_lookup(embed, self.input_placeholder)
        embeddings = tf.reshape(embeddings_temp, shape=[-1, np.prod(embeddings_temp.get_shape().as_list()[1:])])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("parser") as parser_scope:
            ...
            b2 = tf.constant(0.0, shape=[1, self.config.n_classes], dtype=tf.float32)

        input_to_hidden = tf.matmul(x, W) + b1
        h = tf.nn.relu(input_to_hidden)
        h_drop = tf.nn.dropout(h, self.dropout_placeholder)
        h2 = tf.matmul(h_drop, U) + b2
        pred = tf.nn.dropout(h2, self.dropout_placeholder)
        ### END YOUR CODE
        return pred
